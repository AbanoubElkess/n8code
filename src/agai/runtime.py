...
class AgenticRuntime:
    ...
    def run_external_claim_sandbox_campaign(
        self,
        config_path: str,
        registry_path: str | None = None,
        eval_path: str | None = None,
        default_max_metric_delta: float = 0.02,
        sandbox_registry_path: str | None = None,
    ) -> dict[str, Any]:
        ...
        sandbox_dir.mkdir(parents=True, exist_ok=True)
        campaign_sandbox = (
            Path(sandbox_registry_path)
            if sandbox_registry_path
            else (sandbox_dir / "campaign.registry.json")
        )
        payload = self.external_claim_campaign.run(
            eval_report=eval_report,
            source_registry_path=active_registry,
            sandbox_registry_path=str(campaign_sandbox),
